...
pub fn mark_pause_observed(
    ...
    let released_active = state.currently_active_source.as_ref() == Some(source_id);
    if released_active {
...
pub fn remove_source(
    ...
    state.pending_pauses.retain(|_, pause| {
        &pause.paused_source.id != source_id && &pause.requested_by.id != source_id
    });
